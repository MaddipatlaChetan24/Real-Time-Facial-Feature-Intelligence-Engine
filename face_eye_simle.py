...
while True:
    ...
    # detect smile and eyes
    roi_color = frame[y:y+h,x:x+w]
    # text or boxes gets drawn

    eyes = eye_cascade.detectMultiScale(gray,1.1,10)

    if len(eyes) >0:
        cv2.put(frame,"Eyes Detected",(x,y-30),cv2.FONT_HERSHEY_COMPLEX,(0,0,255),2)

    smile = smile_cascade.detectMultiScale(gray,1.7,20)

    if len(smile) >0:
        cv2.put(frame,"Smile Detected",(x,y-10),cv2.FONT_HERSHEY_COMPLEX,(0,0,255),2)
